...
// active\_node 活動節點，主要是用來生長葉節點 (leaf)
// active\_e 隱藏節點的第一個字元
// active\_len 隱藏在 Suffix Tree 中節點的長度
...
//     edge\_length() 公式為 min(end,pos+1)−start
//     用來找出此節點的字串長度
...
void st_extend(char c){ //擴增 suffix tree 
    ...
    while(remainder_ > 0){
        ...
        if(tree[active_node].next[active_edge()] == 0){ 
            ...
            // 需要將 active\_node 指向我們的切割點，而不是原來的點
            int leaf = cnt ; // 需要葉節點
            tree[cnt++].init(pos) ;
            // 製作葉節點
        ...
        if(active_node == root && active_len > 0 ){ 
            //active\_len > 0 表示我們現在做的是把隱藏節點新增，所以要減掉
            //active\_node == root 確保有回到根節點才做隱藏節點減掉，否則 
            //text[node.start + active\_len ] 就會亂掉
            active_len-- ;
            //由於我們減少了一個隱藏長度，所以 -1 
            active_e = pos - remainder_ + 1 ;
            //找到減少後隱藏長度的第一個隱藏字元，此時如果 active\_len == 0，
            // 則下次迴圈則在 active\_e 會被重新定義成 pos
        }
        else{
            // 跟著 slink 走去改動其他的後綴在 tree[active\_node].slink > 0 時，
            // 否則則回到 root，繼續建立後綴樹
